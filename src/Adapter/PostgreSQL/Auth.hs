...
-- | Helper function to execute PostgreSQL-related functions
-- given you are in PG r m context. It acquires a connection
-- from the pool and hands it over to `action`
  ...
  case result of
    ...
    Left err@SqlError{sqlState = state, sqlErrorMsg = msg} ->
      if state == "23505" && "auths_email_key" `isInfixOf` msg
        then return $ Left D.RegistrationErrorEmailToken
